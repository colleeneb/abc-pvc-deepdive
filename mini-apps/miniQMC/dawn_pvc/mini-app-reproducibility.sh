...
source /home/dn-kwac1/mpich-install/source_me_mpich
...
NDEPTH=12          # Number of hardware threads per rank, spacing between MPI ranks on a node
...
do
  mpiexec -np 1 -ppn ${NRANKS} --bind-to $CPU_BIND ../../gpu_tile_compact.sh $build_folder/bin/miniqmc_sync_move -P -g "2 2 1" -w 320 -n 1 > pvc_r1_t8.s$series.out
  mpiexec -np 2 -ppn ${NRANKS} --bind-to $CPU_BIND ../../gpu_tile_compact.sh $build_folder/bin/miniqmc_sync_move -P -g "2 2 1" -w 320 -n 1 > pvc_r2_t8.s$series.out
  mpiexec -np 4 -ppn ${NRANKS} --bind-to $CPU_BIND ../../gpu_tile_compact.sh $build_folder/bin/miniqmc_sync_move -P -g "2 2 1" -w 320 -n 1 > pvc_r4_t8.s$series.out
  mpiexec -np 8 -ppn ${NRANKS} --bind-to $CPU_BIND ../../gpu_tile_compact.sh $build_folder/bin/miniqmc_sync_move -P -g "2 2 1" -w 320 -n 1 > pvc_r8_t8.s$series.out
done
